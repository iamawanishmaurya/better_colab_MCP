...
def isolated_state_path(monkeypatch, tmp_path):
    monkeypatch.setattr(
        "colab_mcp.websocket_server.STATE_PATH",
        tmp_path / "colab-mcp-current.json",
    )
...
async def test_disconnect_notifies_read_stream():
    async with ColabWebSocketServer() as server:
        ...
        receive_task = asyncio.create_task(server.read_stream.receive())

        await client.close()

        received_item = await asyncio.wait_for(receive_task, timeout=1)
        assert isinstance(received_item, Exception)
        assert str(received_item) == "Colab Frontend disconnected"
...
@pytest.mark.asyncio
async def test_token_in_url_must_match_exactly():
    with pytest.raises(
        websockets.exceptions.InvalidStatus,
        check=lambda e: e.response.status_code == 401,
    ):
        async with ColabWebSocketServer() as server:
            await websockets.connect(
                f"ws://localhost:{server.port}?access_token=bad{server.token}",
                origin="https://colab.google.com",
                subprotocols=["mcp"],
            )
